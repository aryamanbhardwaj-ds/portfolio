...
export const ScrollExpand: React.FC<ScrollExpandProps> = ({
  ...
  const propsRef = useRef<ScrollExpandConfig>({
    startWidth,
    startHeight,
    startRadius,
    endRadius,
  ...
  const applyProgress = useCallback((p: number) => {
    ...
    // Blur: starts strong, clears as the frame opens
    const blurAmount = 12 * (1 - smoothstep(0, 0.7, p));
  ...
  const media =
    mediaType === 'video' ? (
      ...
      <div ref={mediaRef} className="scroll-expand__media bg-void" />
    );
